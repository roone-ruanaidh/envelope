...
1. A model may propose a typed plan or bounded frontier expansion when code cannot select one.
2. Code validates each proposal, requires decreasing global potential, and reserves its effects before dispatch.
...
The progression is model-narrated control, model-written executable control, model-proposed typed control, then code-owned control with models only at semantic leaves. Python moves computation before it necessarily moves authority; only validated proposals and code-owned topology constrain what model output can cause.
...
A recursive call stack is convenient runtime state and poor durable state. A crash loses frames, local variables, in-flight ownership, and the exact point at which an effect crossed the process boundary. Recursive semantics should compile into a finite plan or explicit durable frontier whose nodes have stable identities.
...
A model may propose either a complete plan or one frontier expansion. Requiring the complete graph upfront would make evidence-driven decomposition unnecessarily brittle. Each expansion is nevertheless an untrusted candidate: the validator checks it against current state, atomically reserves its effects, and requires it to consume a finite global potential such as remaining calls, cost, depth, or problem rank. Adaptation and backtracking remain possible, but no proposal gains open-ended control authority.
...
Its near-term value may therefore be the design lesson—parallelism follows recursive program shape—more than the language dependency itself.
...
The progression should be empirical. Do not begin by designing a universal recursive DSL. Capture real RLM trajectories, identify repeated control structures, and promote the smallest operator set that removes the largest amount of model-authored control. Formalize the algebra in Lean only after that set stabilizes; otherwise the proof effort fossilizes an untested boundary. An operator earns promotion when its semantics, resource behavior, and evidence are precise enough to test and eventually prove.

### Evaluation measures

Accuracy alone cannot show whether control actually moved into code. Measure context amplification (external bytes processed per byte shown), orchestration-token fraction, model-authored branch count, unreserved exposure, semantic-leaf ratio, replay equivalence under reordered or duplicated events, and p95/p99 calls, cost, and latency. The desired result is not merely a higher mean score: it is less model-authored control and narrower cost tails without losing semantic quality.
...
The sharpest unresolved problem is adaptive semantic decomposition:

> What is the smallest incrementally expandable plan algebra that preserves useful RLM adaptation while making termination, authority, budget, and replay mechanically enforceable?
...
3. Represent recursion as a validated finite plan or incrementally validated frontier that consumes finite global potential.
