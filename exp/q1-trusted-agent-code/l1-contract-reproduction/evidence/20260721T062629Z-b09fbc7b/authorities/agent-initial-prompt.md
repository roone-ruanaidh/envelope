...
Your final response must match the supplied JSON schema. Return `declared_complete` only after the workspace contains the candidate you intend to evaluate, with `service_command` as the exact foreground argv to run from the workspace root. Preserve empty non-executable arguments when they are intentional. If you cannot complete without an undefined decision or unavailable authority, return `blocked` with the reason. Do not ask a question.
